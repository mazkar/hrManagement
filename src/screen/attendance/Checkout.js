...
export default function ClockOut({ navigation, route, punchId }) {
  ...
  async function handleClockOut() {
    ...
    const data = {
      employeeId: userData?.hrEmployeeId,
      LMBY: userData?.uid,
      punchId: route?.params?.punchId,
      punchOutLongitude: route?.params?.longitude.toString(),
      punchOutLatitude: route?.params?.latitude?.toString(),
      photoBase64: image,
      punchOutDistance: distance.toString(),
    };
    console.log(data, "body");
    try {
      // console.log(img._j, "body");
